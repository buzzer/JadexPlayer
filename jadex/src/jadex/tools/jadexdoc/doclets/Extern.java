...
public class Extern
{
	...
	private class Item
	{
		 ...
		 * @param pkgName Package name found in the "package-list" file.
		 * @param path URL or Directory path from where the "package-list"
		 * file is picked.
		 * @param relative True if path is URL, false if directory path.
		 */
		Item(String pkgName, String path, boolean relative)
		{
			//System.out.println("FOUND: "+pkgName+" "+path+" "+relative);
			...
			if(!packageToItemMap.containsKey(pkgName))
			{ // save the previous
				packageToItemMap.put(pkgName, this);        // mapped location
			}
		}
	...
	public Extern(StandardConfiguration configuration)
	{
			...
			boolean linkoffline)
	{
		...
		if(errMsg!=null)
		{
			configuration.message.printError(errMsg);
			return false;
		}
		else
		{
	...
	private String readFileComposeExternPackageList(String path, String pkgListPath)
	{
		...
		if(!new File(pkgListPath).isAbsolute() && !linkoffline 
			&& new File(configuration.destdirname).isAbsolute())
		{
...
//						if(path.startsWith("."))
//							System.out.println("readPackageList: "+path+" "+packname+" "+packpath);
						strbuf.setLength(0);
					}
				}
				else
