...
public class Group
{
	...
	public boolean checkPackageGroups(String groupname, String pkgNameFormList)
	{
		...
		{
			msg().printError(msg().getText("doclet.Groupname_already_used", groupname));
			return false;
		}
		...
		while(strtok.hasMoreTokens())
		{
			...
			if(id.length()==0)
			{
				msg().printError(msg().getText("doclet.Error_in_packagelist",
	...
	boolean foundGroupFormat(Map map, String pkgFormat)
	{
		...
		{
			msg().printError(msg().getText("doclet.Same_package_name_used", pkgFormat));
			return true;
		}
		return false;
	}
	...
	public Map groupPackages(String[] packages)
	{
		...
		for(int i = 0; i<packages.length; i++)
		{
			...
			getPkgList(groupPackageMap, groupName).add(pkgName);
		}
