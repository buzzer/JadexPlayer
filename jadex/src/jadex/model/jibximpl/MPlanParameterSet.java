 ...
 */
public class MPlanParameterSet extends MParameterSet implements IMPlanParameterSet
{
	//-------- xml-attributes --------
	...
	protected void doCheck(Report report)
	{
		...
		checkMappings(getGoalMappings(), plan.getTrigger()!=null?
			plan.getTrigger().getGoals(): null, report, getScope().getGoalbase());
	...
	protected void checkMappings(String[] mappings, IMReference[] triggers, Report report, IMBase base)
	{
		if(mappings.length>0)
		{
			...
			for(int i=0; i<mappings.length; i++)
			{
				...
				String paramsetname = mappings[i].substring(idx+1);
				IMReferenceableElement elem = base.getReferenceableElement(trname);

				if(!findParameterSet(elem, paramsetname))
					report.addEntry(this, "Parameterset of trigger not found: "+paramsetname);
			}
		}
	}
	 ...
	 */
	protected boolean findParameterSet(IMReferenceableElement elem, String paramsetname)
	{
		boolean success = false;

		if(elem instanceof IMParameterElement)
		{
			if(((IMParameterElement)elem).getParameterSet(paramsetname)!=null)
			{
				success = true;
			}
		}
		...
		{
			if(((IMParameterElementReference)elem).getParameterSetReference(paramsetname)!=null)
			{
				success = true;
			}
			else
			{
				...
				success = findParameterSet(((IMParameterElementReference)elem).getReferencedElement(), paramsetname);
			}
		}
		return success;
	}
