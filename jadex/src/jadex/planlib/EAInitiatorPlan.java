...
public class EAInitiatorPlan extends AbstractInitiatorPlan
{
	...
	protected IMessageEvent announceAuction(Object auctiondesc, List receivers, String convid)
	{
		...
		IMessageEvent me = getEventbase().createMessageEvent("ea_inform_start_auction");
	...
	{
		AgentIdentifier first_proposal = null;
		...
		while(System.currentTimeMillis() - roundstart < roundtimeout)
		{
			try
			{
				...
				IMessageEvent tmp = (IMessageEvent)waitFor(mf, roundtimeout);
				...
				if(tmp.getType().equals("ea_propose"))
				{
					if(first_proposal==null)
					{
						...
						sendMessage(accept);
						first_proposal = (AgentIdentifier)tmp.getParameter(SFipa.SENDER).getValue();
					}
					else
					{
