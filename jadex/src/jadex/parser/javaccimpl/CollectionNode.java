...
public class CollectionNode	extends ExpressionNode
{
	...
	public void precompile()
	{
		...
		// Precompute type.
		Class	clazz	= collnode.getStaticType();
	...
	public Object	getValue(Map params) throws Exception
	{
		...
		ExpressionNode	collnode	= (ExpressionNode)jjtGetChild(0);
		ExpressionNode	argsnode	= (ExpressionNode)jjtGetChild(1);
		Object	collection	= collnode.getValue(params);
		...
		for(int i=0; i<argsnode.jjtGetNumChildren(); i++)
		{
			...
			if(node instanceof ArgumentsNode)
			{
				...
				((Map)collection).put(key.getValue(params),
					val.getValue(params));
			}
			...
			{
				((Collection)collection).add(node.getValue(params));
			}
		}
